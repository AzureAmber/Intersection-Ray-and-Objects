...
class vector:
    ...
    def normalize(self):
        mag = self.length()
        return(vector(self.x / mag, self.y / mag, self.z / mag, self.w))
    ...
    def __add__(self, v):
        if isinstance(v, float):
            return vector(self.x + v, self.y + v, self.z + v, self.w)
        elif isinstance(v, vector):
            return vector(self.x + v.x, self.y + v.y, self.z + v.z, self.w)
        else:
    ...
    def __radd__(self, v):
        if isinstance(v, float):
            ...
            return vector(self.x + v.x, self.y + v.y, self.z + v.z, v.w)
        else:
            raise TypeError('add: Not float or vector')
    def __sub__(self, v):
        if isinstance(v, float):
    ...
    def __mul__(self, v: float):
        return vector(v * self.x, v * self.y, v * self.z, self.w)
    ...
    def matmult(self, M: matrix):
        ...
        for i in range(4):
            ...
            result[i] = temp
        w = vector(result[0], result[1], result[2], result[3])
        return w
